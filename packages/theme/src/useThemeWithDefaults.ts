...
export const getThemeWithDefaults = (colorScheme, userTheme) => {
  ...
  return extendTheme(
    ...colorScheme.componentsByColorScheme.map(item => withDefaultColorScheme(item)),
...
export const useThemeWithDefaults = (colorScheme, userTheme) => {
  const customTheme = useMemo(() => getThemeWithDefaults(colorScheme, userTheme), [colorScheme, userTheme]);
  return customTheme
};
